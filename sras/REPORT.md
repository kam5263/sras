...
| **SRAS (PPO best)** | Warmup 이후 PPO epoch 9 최고 보상 체크포인트 |
...
**[Figure 1] Epoch별 평균 보상 (avg_reward)**

```
reward
0.595 |                  ★ 0.5929 (best, epoch 9)
      |              ●●●●
0.585 |          ●●●●    ●
      |        ●●          ●●●●
0.575 |      ●●                ●●●
...
Epoch 9에서 평균 보상이 0.5929로 최고치를 기록한 후 단조 감소. 이후 policy collapse 징후.
...
Hit rate도 epoch 9(93.6%)를 정점으로 이후 진동하며 감소.

---

**[Table 1] Epoch별 Hit/Miss 보상 분리 추이**

| Epoch | avg_reward | Hit Rate | R(hit) | R(miss) | Gap |
|---|---|---|---|---|---|
| 1 | 0.5781 | 86.5% | 0.625 | 0.280 | 0.345 |
| 5 | 0.5821 | 89.9% | 0.623 | 0.237 | 0.386 |
| 9 | 0.5929 | 93.6% | 0.614 | 0.287 | **0.327** |
| 15 | 0.5672 | 91.3% | 0.599 | 0.245 | 0.354 |
| 20 | 0.5561 | 90.9% | 0.588 | 0.243 | 0.345 |
| 25 | 0.5371 | 87.9% | 0.577 | 0.245 | 0.332 |

Gold doc 선택 시(R(hit))와 미선택 시(R(miss))의 보상 차이 ~0.33이 전 epoch에 걸쳐 유지됨. 모델이 검색 품질과 보상 신호의 관계를 올바르게 학습하고 있음을 확인.
...
**[Figure 3] 테스트셋 Hit Rate @3 시각 비교**
...
**발견 1: PPO가 Warmup 모델을 degradation시킴**

```
Warmup (97.2%) ──PPO 25 epochs──▶ PPO best (86.8%)
                                        ▼ -10.4pp
```

PPO 훈련이 Hit Rate를 훈련셋에서 97.2% → 86.8%, 테스트셋에서 90.0% → 79.5%로 하락시켰다. 이는 PPO 보상 신호(Ollama 생성 답변의 F1+BERTScore)가 실제 검색 정확도와 충분히 정렬되지 않았음을 의미한다. 25 epoch의 노이즈 신호가 warmup에서 학습된 유용한 표현을 점진적으로 파괴하는 패턴은 RLHF 연구에서 보고된 reward misalignment 현상과 일치한다.
...
**발견 3: Random distractor의 한계**

현재 candidate pool(1 gold + 7 완전 랜덤)에서는 Cosine 유사도만으로도 쉽게 gold doc을 상위에 랭킹할 수 있다(96.2%). 이 환경에서는 SRAS가 Cosine 대비 추가 학습 이점을 얻기 어렵다. 어려운 negative sample 없이는 Cosine 이상의 판별력을 학습할 유인이 부족하다.
...
SRAS는 엣지 배포 조건(0.38MB, 0.036ms CPU 지연)을 만족하면서, Supervised Warmup만으로 테스트셋 Hit Rate @3 **90.0%** 를 달성하였다. 이는 랜덤 baseline(44.0%) 대비 +46pp의 성능 향상이며, Cosine baseline(94.0%)과 4pp 차이에 해당하는 경쟁력 있는 결과이다.

그러나 PPO 강화학습 훈련은 보상 신호의 노이즈 및 misalignment로 인해 Hit Rate를 오히려 10pp 이상 저하시켰다. 향후 **KL-regularized PPO** 와 **Hard Negative Sampling** 을 적용하면 강화학습의 이점을 실제로 활용할 수 있을 것으로 기대된다.
...
├── checkpoints/
│   ├── sras_warmup.pt      # Warmup 체크포인트 ← 최고 성능
│   └── sras_best.pt        # PPO epoch 9 체크포인트
├── data/
